...
In this document you will find rules for how to use the language, intended to help construct a consistent and portable usage of the language across many use cases and implementations.
...
In case two rules are in conflict, and there is no explicit mention which rule trumps, the last mentioned rule applies.
...
.. No padding space on the inside of function call parentheses.
+
.Bad
[source, cypher]
----
...
** When referring to labels and relationship types, include the colon like so: `:Label`, `:REL_TYPE`.
** When referring to functions, use lower camel case and parentheses like so: `shortestPath()`. Arguments should normally not be included.
