...
Feature: UnwindAcceptance
  ...
  Scenario: [9] Unwinding list with duplicates
    Given any graph
    When executing query:
      """
  ...
  Scenario: [10] Unwind does not prune context
    Given any graph
    When executing query:
      """
      WITH [1, 2, 3] AS list
  ...
  Scenario: [1] Unwind does not remove variables from scope
    Given an empty graph
    And having executed:
      """
      CREATE (s:S),
  ...
  Scenario: [11] Multiple unwinds after each other
    Given any graph
    When executing query:
      """
