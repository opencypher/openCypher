...
For example, given the same data as previously, “++MATCH (a)-->(b) WHERE a.name = "Bob"++” would only match connected pairs of nodes where the first of that pair has a name propery of ‘Bob’. ie:
