...
* Nullable types are all types that permit `NULL` as a valid result of evaluating the underlying expression.
  Nullable types are formed by suffixing a non-nullable type with a question mark.
  Note that `NULL` is not a type but a value that inhabits every nullable type.

As a consequence of this, `ANY?` is the most generic type available within Cypher and is the supertype of every possible value.
