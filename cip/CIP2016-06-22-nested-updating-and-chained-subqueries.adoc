...
Chained subqueries are queries that compose two (argument) subqueries using a _query combinator_ clause.

While (full) top-level queries are arbitrary Cypher queries, argument queries are queries that may not contain query combinators.
They may however contain nested subqueries which may use query combinators in their inner queries (which again may be top-level queries).
...
This CIP proposes that all Cypher query combinators are left-associative regarding their argument queries.
