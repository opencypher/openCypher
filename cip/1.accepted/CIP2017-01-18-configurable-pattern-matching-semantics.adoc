...
IMPORTANT: This CIP proposes that the default pattern variable class is `TRAIL` with the exception that any path pattern part is to be matched separately using the `WALK` pattern variable class.
...
* Simple relationship patterns (e.g. `()-[]\->()`) are to be matched using `ALL`.
...
This CIP finally proposes a function `disjoint()` defined as follows:

* `disjoint(list1, list2, ..., list_n)`: returns `true` if all argument lists are pairwise disjoint
* If any argument is a walk, it is treated as a list of alternating nodes and relationships
