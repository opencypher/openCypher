...
* `DEFAULT [GRAPH] [<new-graph-name>]`: Reference to the _default graph_, optionally to be bound as `<new-graph-name>`
...
=== Discarding the current source and target graphs
...
It is additionally proposed that both `WITH GRAPHS <graph-return-items>` and `RETURN GRAPHS <graph-return-items>` may be used for discarding all tabular data such that the provided tabular input for the following clause (or query respectively) would again be the provided single record without any fields as specified by the rules for partial query contexts.
