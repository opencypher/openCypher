...
The data model underpinning Cypher is the _property graph model_.
...
A _**label**_ is used to classify a node.

A _**relationship type**_ is a label that is used to classify a relationship.
...
A _**reference**_ is a handle (i.e. an otherwise opaque value) that is used to address a model element of a property graph model instance (i.e. a node or a relationship).
...
6. Cypher syntax is modal: Not all clauses are allowed in all contexts.
