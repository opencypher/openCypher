...
* `NEW GRAPH [AT <graph-url>] [AS <new-graph-name>]`: Introduce a newly created, empty graph that is to be made available at the provided `<graph-url>`
...
* `GRAPH AT <graph-url> [AS <new-graph-name>]`: Load and introduce the graph that is available at the provided `<graph-url>`

The alias of a `<graph-def>` is either the explicitly given `<new-graph-name>` or `<graph-name>` if present or an implicit fresh system generated name otherwise.
...
INTO NEW GRAPH AT 'graph://...' AS result
...
INTO NEW GRAPH AS berlin
CREATE (a)-[:FRIEND]->(b) WHERE c.name = "Berlin"
INTO NEW GRAPH AS santiago
...
INTO NEW GRAPH AS recommendations
...
INTO NEW GRAPH AT './swe' AS sweden_people {
...
INTO NEW GRAPH AT './ger' AS german_people {
...
INTO NEW GRAPH AS swedish_triangles {
  MERGE p
}
...
[source, cypher]
----
FROM GRAPH AT 'graph://actors_films_cities...' AS ActorsFilmsCities
----
...
[source, cypher]
----
INTO NEW GRAPH AT 'some-location' AS PersonCityEvents
----
...
[source, cypher]
----
FROM GRAPH AT 'graph://events...' AS Events
MATCH (c)<-[:IN_CITY]-(e)-[:IN_YEAR]->(y),
      (e)-[:IS_A]->(et {value: 'Criminal Event'})
...
[source, cypher]
----
INTO NEW GRAPH AS Temp-PersonCityCrimes
----
