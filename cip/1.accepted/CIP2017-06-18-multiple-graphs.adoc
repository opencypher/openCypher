...
This CIP proposes redefining the *execution context* to be
...
* optional information indicating which of these named graphs, if any, is the *source graph*
* optional information indicating which of these named graphs, if any, is the *target graph*
...
A parameterized Cypher query under this model can _also_ be described as executing within (and operating on) a given execution context and an initial query context and finally returning the query context produced as output for the top-most `RETURN` clause.
...
* `SOURCE GRAPH [<new-graph-name>]`: The currently _provided source graph_
* `TARGET GRAPH [<new-graph-name>]`: The currently _provided target graph_
* `DEFAULT GRAPH [<new-graph-name>]`: The _default graph_

If no `<new-graph-name>` is provided in the input query, a fresh name is generated implicitly.
...
FROM < graph-spec >
INTO < graph-spec >
COPY GRAPH < graph-ref > [ TO < graph-url > ] AS < new-graph-name >
----

==== DEFINE clause

This CIP proposes the `DEFINE` clause for introducing new graphs using the form `DEFINE <graph-def>`.
...
This CIP proposes a new `COPY GRAPH` clause to copy graphs using the form `COPY GRAPH <graph-ref> [TO <graph-url>] AS <new-graph-name>`.
...
If the current named source graph (or the current named target graph) are not passed on, they are discarded, and owing to the rules regarding partial query contexts, the provided source graph (or target graph respectively) again are chosen to be the default graph of the outer execution context.
