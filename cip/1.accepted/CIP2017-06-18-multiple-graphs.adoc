...
This CIP first proposes new syntactical concepts for them before proceeding to add new and extend existing clauses.
...
It is an error to use the same name for both a regular variable and a graph at the same time.
...
This CIP defines the notion of a graph definition `<graph-def>` as a means to introduce additional named graphs.
...
This CIP proposes to the `DEFINE` clause for introducing new graphs using the form `DEFINE <graph-def>`.
...
Note: It may seem odd to specify a source graph for clauses like `SET`.  However, the source graph is used to evaluate all expressions (exception: `MERGE`) and therefore being able to override it temporarily may be useful.
