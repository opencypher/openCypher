...
For procedure that does not return any result fields, the query will return as many (empty) rows as produced by the called procedure.

Another subclause allowed in the standalone form is `YIELD *`, which denotes that all result fields produced by the procedure implementation are returned by the statement.
For procedure that does not return any result fields, the query will return as many (empty) rows as produced by the called procedure.
...
Standalone calls that use the `YIELD *` subclause will always project all result fields.
...
Both simplifications may be used in a single standalone procedure call, leading to a concise syntax for just executing a single procedure call:

.Standalone procedure call with `YIELD *`
[source, cypher]
----
CALL myProc YIELD *
----

===== Calling with implicitly passed arguments (parameters) and omitting `YIELD`

As a further simplification, the `YIELD` subcluase can be omitted alltogether from a standalone procedure call, leading to an even more concise syntax:
...
==== Standalone call with omitted `YIELD` clause

Standalone calls without `YIELD` are supported for all procedures:

.Standalone procedure call with omitted `YIELD` and explicitly passed arguments
[source, cypher]
----
CALL myProc($arg)
----
