...
A further simplification allowed in the standalone form is to use `YIELD *` to denote that all result fields produced by the procedure implementation are returned by the statement.
...
|CALL proc(..) YIELD *     |Standalone  |Explicit        |New Fields|All
|CALL proc YIELD *         |Standalone  |Implicit        |New Fields|All
