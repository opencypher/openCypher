...
Such chained operators fall into four categories:

==== Chaining type-incompatible operators
...
[IMPORTANT]
.Chaining of type-incompatible operators
====
...
==== Chaining closed unary operators
...
====
Where the semantics of operator is associative (e.g. addition), a right-deep operator tree produces the same result.
====
...
openCypher has grammatically all operator in one precedence level hierarchy.
...
* https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Operator_Precedence#table[Java Script]
