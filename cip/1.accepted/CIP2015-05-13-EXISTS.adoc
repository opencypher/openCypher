...
If the input expression returns a +null+ value, +EXIST(...)+ returns false, and for all other values, it will return +true+.
This form of +EXISTS(...)+ is equivalent to checking for +null+ using +IS NOT NULL+.
I.e. +EXISTS(expr)+ is equivalent to +expr IS NOT NULL+.
